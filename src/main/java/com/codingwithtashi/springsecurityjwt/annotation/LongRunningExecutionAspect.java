...
public class LongRunningExecutionAspect {
    ...
    public Object logMethod(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        boolean isMaxExecutionTimeFilter = Objects.nonNull(longRunningExecution)
                && longRunningExecution.maxExecutionTime() > -1L;
        ...
        if (isMaxExecutionTimeFilter) {
            if (methodExecutionTimeInMs > longRunningExecution.maxExecutionTime()) {
                if (isRequestLogging) {
                    log.info(
                            "LongRunningExecution: {} - Request: {} - Response time {} ms - Expected Response time {} ms",
                            methodPath, joinPoint.getArgs(), methodExecutionTimeInMs,
                            longRunningExecution.maxExecutionTime());
                }
                if (isResponseLogging) {
                    log.info(
                            "LongRunningExecution: {} - Response: {} - Response time {} ms - Expected Response time {} ms",
                            methodPath, result, methodExecutionTimeInMs, longRunningExecution.maxExecutionTime());
                }
            }
        } else {
            if (isRequestLogging) {
                log.info("LongRunningExecution: {} - Request: {} - Response time {} ms", methodPath,
                        joinPoint.getArgs(), methodExecutionTimeInMs);
            }
            if (isResponseLogging) {
                log.info("LongRunningExecution: {} - Response: {} - Response time {} ms", methodPath, result,
                        methodExecutionTimeInMs);
            }
        }
        return result;
    }
